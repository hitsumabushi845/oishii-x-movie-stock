...
- フリーワード検索（本文 + 手動付与 tags、クライアントサイド）
- 並び順切り替え（新しい順 / 古い順）
- 動画長フィルタ（1 分以上のみ表示）
- 無限スクロール
...
| 項目 | 決定 | 理由 |
|---|---|---|
| データ取得 | X API v2 search/all（`from:official_aimai has:videos -is:retweet`）、Bearer Token | サーバ側で動画ツイートだけに絞って取得、クレジット消費を最小化 |
...
- Python 3.12、uv 管理
- データ取得本体: **X API v2**（`httpx` で直接呼び出し）
- 補助: `pydantic` v2、`jsonschema`

### データ取得方式（X API v2 search/all）

`GET /2/tweets/search/all` を使い、サーバ側で動画ツイートだけに絞って取得する。

- 検索クエリ：`from:official_aimai has:videos -is:retweet`
  - `from:` で対象アカウントを限定
  - `has:videos` で動画を含むツイートのみに限定。`filter:native_video` は dev tier 利用不可だったため `has:videos` に切替（RT/引用 RT が混ざる場合があるが、media expansion で video が取れないものは defensive にスキップする）
- pagination：`next_token` で全ページ取得
- インクリメンタル：`start_time={last_synced_at}` を付与して差分のみ取得
- バックフィル：`start_time={BACKFILL_EPOCH}`（= 2010-01-01）を付与して全期間取得。**search/all は `start_time` 無指定だと約 30 日のデフォルト窓に縛られる**ため明示的に過去日付を指定する必要がある
- user lookup 不要（`from:` で username を直接指定）
...
1. `data[]` の各 tweet について `includes.media` を media_key で参照
2. `type == "video"` のメディアを 1 つでも持つものを採用（防御的：filter が効いていれば不要だが念のため）
3. video media の `duration_ms` を 1000 で割って秒に丸めて `duration_sec` とする
4. 投稿 URL は `https://x.com/official_aimai/status/{tweet.id}` を組み立て

### user_timeline ではなく search/all にした理由

| 観点 | user_timeline | search/all |
|---|---|---|
| user lookup | 必要 | 不要 |
| 動画フィルタ | クライアント側で型判定 | サーバ側で `filter:native_video` |
| 1 リクエストあたり | 写真・テキスト含む 100 件 | 動画ツイートのみ 100 件 |
| バックフィル時のリクエスト数 | 動画密度に応じて増える（最大 3,200/100=32） | 動画件数 ÷ 100 のみ |

`@official_aimai` の動画密度が低い（写真・テキスト中心の運用）ことを踏まえ、search/all のほうがクレジット消費が圧倒的に少ない。tier 制約は現在無し（クレジット残高のみが利用可否を決める）。
...
```
 ...
 2. クエリ組み立て:
      base = "from:official_aimai has:videos -is:retweet"
...
- **Bearer Token のみ**（X API v2 App-only auth）
- 環境変数 `X_BEARER_TOKEN` で渡す
- GitHub Actions では Secrets `X_BEARER_TOKEN` から env に渡す
- ローカル開発時は `scraper/.env`（`.gitignore` 済み）に書く

### クレジット消費の見積もり

X API はリクエスト単位で credits を消費する。本プロジェクトの想定：

- 初回バックフィル：動画件数 ÷ 100 リクエスト（100 件規模なら 1〜2 リクエスト）
- 週1 cron：差分のみ取得、新規動画 0〜数件なら 1 リクエスト/週
- user lookup：不要（`from:` で username 指定）

tier 制約は無く、クレジット残高のみで決まる。Free tier でも残高さえあれば動作。
...
- Secrets: `SLACK_WEBHOOK_URL`、`X_BEARER_TOKEN`
...
- **X API クレジット枯渇**：想定外のリトライや tier 変更で credits が尽きると 402 で停止。Slack 通知で気付ける
- **X API 仕様変更**：endpoint・フィールドが変わる可能性。週1 cron が長期間失敗し続けたら見直す
- **埋め込み widgets.js**：将来 X 側で変更される可能性。再生面のリスク（インデックス自体は機能継続）
- **Bearer Token 漏洩**：Secrets に保存、定期ローテーションは X Developer Portal で実施可
