...
| 項目 | 決定 | 理由 |
|---|---|---|
| データ取得 | X API v2（user timeline + media expansion）、Bearer Token | snscrape 系の不安定さを避け、公式 API のクレジット課金で運用 |
...
`from:official_aimai filter:native_video` ベースの search ではなく、**user timeline + media expansion** に振った理由：

- search recent endpoint は過去 7 日制限があり、バックフィルに使えない
- user timeline (`GET /2/users/:id/tweets`) は最大 3,200 件まで遡れて、`since_id` / `start_time` で増分取得もできる
- media expansion (`expansions=attachments.media_keys`、`media.fields=type,duration_ms`) で video 判定が完結

呼び出し手順：

1. 初回のみ `GET /2/users/by/username/official_aimai` で user_id を取得し、定数として保持（または起動時 1 回 lookup）
2. `GET /2/users/{user_id}/tweets` を `pagination_token` で next_page しつつ全ページ取得
   - `max_results=100`
   - `tweet.fields=created_at,text,attachments`
   - `expansions=attachments.media_keys`
   - `media.fields=type,duration_ms`
   - インクリメンタル時は `start_time={last_synced_at}` を付与
3. 各 tweet について、`includes.media` を media_key で参照、`type == "video"` のメディアを 1 つでも持つものを採用
4. video media の `duration_ms` を 1000 で割って秒に丸めて `duration_sec` とする
...
- 初回バックフィル：100 件規模なら 1〜2 リクエスト（max_results=100、pagination 1〜2 回）
- 週1 cron：差分のみ取得、ほぼ 1 リクエスト/週
- user lookup：起動毎に 1 回（または cache 化で削減可能）

Basic tier ($200/mo) の付与クレジットなら余裕。Free tier はクレジット手動購入が必要なケースあり。
