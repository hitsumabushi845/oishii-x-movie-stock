...
| 項目 | 決定 | 理由 |
|---|---|---|
| データ取得 | X API v2 search/all（`from:official_aimai filter:native_video`）、Bearer Token | サーバ側で動画ツイートだけに絞って取得、クレジット消費を最小化 |
...
- 検索クエリ：`from:official_aimai filter:native_video`
  - `from:` で対象アカウントを限定
  - `filter:native_video` で X が直接ホストする動画ツイートのみに限定（外部リンク・GIF・写真は除外）
- pagination：`next_token` で全ページ取得
- インクリメンタル：`start_time={last_synced_at}` を付与して差分のみ取得
- バックフィル：`start_time` 無指定でアカウント開設以降全件
