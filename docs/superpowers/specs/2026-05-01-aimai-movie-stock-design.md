...
| 項目 | 決定 | 理由 |
|---|---|---|
| データ取得 | X API v2 search/all（`from:official_aimai has:videos`）、Bearer Token | サーバ側で動画ツイートだけに絞って取得、クレジット消費を最小化 |
...
- 検索クエリ：`from:official_aimai has:videos`
  - `from:` で対象アカウントを限定
...
```
 ...
 2. クエリ組み立て:
      base = "from:official_aimai has:videos"
